...
int CHarrisCornerApp::run2(int argc, char const *argv[])
{
    if (argc < 2)
    {
        printf("./HarrisCorner <<image>>\r\n");
        return 0;
    }
    ...
    if (!img.empty())
    {
        ...
        cornerHarris(imgGray, dst, 2, 3, 0.04);
        normalize(dst, dst_norm, 0, 255, NORM_MINMAX, CV_32FC1, Mat());
        convertScaleAbs(dst_norm, dst_norm_scaled);
        ...
        namedWindow(szWin, CV_WINDOW_AUTOSIZE);
        imshow(szWin, dst_norm_scaled);
        waitKey();
        // cornerHarris(imgGray, dst, 2, 3, 0.04);
    }
